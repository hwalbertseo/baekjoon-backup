...
void comb(int x1, int y1, int x2, int y2){
    ...
    if(sz[q.first][q.second] < sz[p.first][p.second]){
        pair<int, int> temp;
        temp.first = q.first;
    ...
    sz[q.first][q.second] += sz[p.first][p.second];
...
void melt(int x, int y){
    ...
    while(!q.empty()){
        ...
        for(int i = 0;i < 4;i++){
            ...
            if(visit.count({nx,ny}) == 1){
                continue;
            }
            if(arr[nx][ny] == 'X' || changed.count({nx, ny}) == 1){
                arr[nx][ny] = '.';
                changed.insert({nx, ny});
...
void make(int x, int y){
    ...
    while(!q.empty()){
        ...
        q.pop();
        if(visit.count({curx,cury}) == 1){
            continue;
        }
        visit.insert({curx, cury});
        ss[curx][cury] = {x, y};
        for(int i = 0;i < 4;i++){
            int xx = curx + dx[i];
            int yy = cury + dy[i];
...
int main()
{
    ...
    while(1){
        ...
        for(pair<int, int> p : best){
            if(bfs.count({p.first,p.second}) == 1){
                continue;
            }
            bfs.insert({p.first, p.second});
